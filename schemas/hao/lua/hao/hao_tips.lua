...
function tips_db.get_hao_version()
    return tips_db.meta_fetch(META_KEY_VERSION)
end

---@param version string
function tips_db.update_hao_version(version)
...
local function init_tips_userdb()
    local has_preset_tips_changed = hao.version ~= tips_db.get_hao_version()
    ...
    tips_db.update_hao_version(hao.version)
    tips_db.update_user_tips_file_hash(user_tips_file_hash or "")

    local preset_file_path = hao.get_filename_with_fallback(FILENAME_TIPS_PRESET)
...
function P.func(key, env)
    ...
    if not is_tips_enabled or not segment then
        return hao.RIME_PROCESS_RESULTS.kNoop
    end
    ...
    if not P.tips_key                                   -- 未设置上屏键
        ...
        or hao.is_function_mode_active(context)    -- 或者是功能模式不用上屏
    ...
    if commit_txt and #commit_txt > 0 then
        ...
        context:clear()
        return hao.RIME_PROCESS_RESULTS.kAccepted
    end
