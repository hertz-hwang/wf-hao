...
local function get_tricomment(cand, env, spll_rvdb)
  ...
  if utf8.len(text) == 1 then
    ...
    return env.engine.context:get_option('hao_spelling.lv1')
        ...
        or env.engine.context:get_option('hao_spelling.lv2')
        and xform(raw_spelling:gsub('%[(.-,.-),.*%]', '[%1]'))
        or xform(raw_spelling) -- hao_spelling.lv3 is on
  ...
  elseif utf8.len(text) > 1 then
    ...
    if env.engine.context:get_option('hao_spelling.lv1') then
...
local function filter(input, env)
  if env.engine.context:get_option('hao_spelling.off') then
